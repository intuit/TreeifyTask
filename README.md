...
```C#
// Fixed structure construction
var rootTask =
    new TaskNode("Root", null, ExecutionMode.Series,
        new TaskNode("Task1", Task1_Action, ExecutionMode.Series,
            new TaskNode("Task1.1", Task1_1_Action),
            new TaskNode("Task1.2", Task1_2_Action)),
        new TaskNode("Task2", null, ExecutionMode.Series,
            new TaskNode("Task2.1", null, ExecutionMode.Series,
                new TaskNode("Task2.1.1", Task2_1_1_Action),
                new TaskNode("Task2.1.2", Task2_1_2_Action)),
...
This component should support dynamic number of nodes that gets generated from a Json/Xml content. Targeted syntax below

``` json
[
    {
        "TaskNodeId": "Root",
        "PreferredExecutionMode": "Series",
        "Children": [
            {
                "TaskNodeId": "Task1.1",
                "PreferredExecutionMode": "", // this gets inherited from parent or can have a value.
                "Action": {
                    // Action integration details for example to specify Assembly -> Type -> List<MethodInfo> -> "MethodName", 
                    // The specified method will be loaded by reflection
                }
            }
        ]
    }
...
``` C#
var rootTask = TaskNode.Parse(content, ContentType.Json);

rootTask.Execute(cancellationToken, throwOnError: true/false);
```
